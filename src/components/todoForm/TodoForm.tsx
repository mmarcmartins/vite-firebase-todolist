  ...
  return (
    <StyledForm>
        <div className="input-styled">
          <input value={todo} placeholder="ADD NEW TODO" onKeyPress={handleKeyPress} onChange={(event) => setTodo(event.target.value)}/>
          <button onClick={handleSetTodo} className="mobile-addtodo-button">Press me to add a new TODO</button>
        </div>        
    </StyledForm>
  )
}
