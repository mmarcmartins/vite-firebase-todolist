...
import { TodoInterfacePreview } from './TodoService.d';
...
export default class TodoService{
  ...
  async removeTodoFromDd(id: string) : Promise<boolean>{
    const [error] = await handlePromise(deleteDoc(doc(this.#todosCol,id)))    
    if(error){
  ...
  async completeTodo(id: string) : Promise<boolean>{
    ...
    const [error] = await handlePromise(updateDoc(ref, { isComplete: true }));
    return error || true;
  }
